...
from luscious_dl.parser import extract_ids_from_list, \
        extract_album_id, extract_user_id
from luscious_dl.user import User
from luscious_dl.utils import info, format_foldername, \
        generate_pdf, inputs_string_to_list, delete_folder, \
        generate_cbz, read_list


def albums_download(albums_ids: list[int],
                    downloader: Downloader, output_dir: Path,
                    foldername_format='%t',
                    gen_pdf=False, gen_cbz=False, rm_origin_dir=False) -> None:
    """
    ...
    for id_ in albums_ids:
        ...
        try:
            if album.fetch_info():
                formmatted_foldername = format_foldername(
                    album, foldername_format)
                album_folder = output_dir.joinpath(formmatted_foldername)
                album.show()
                ...
                if gen_pdf:
                    generate_pdf(
                        output_dir, formmatted_foldername, album_folder)
                if gen_cbz:
                    generate_cbz(
                        output_dir, formmatted_foldername, album_folder)
                if rm_origin_dir:
            ...
            if user.fetch_info():
                ...
                if group_by_user:
                    output_dir = output_dir.joinpath(
                        os.path.normcase(user.name))
                albums_download(user.albums_ids, downloader, output_dir,
                                foldername_format, gen_pdf, gen_cbz, rm_origin_dir)
            else:
...
def start(args: Namespace = None) -> None:
    ...
    downloader = Downloader(args.threads, args.retries,
    ...
    if args.albums_ids:
        ...
        result = search_albums(args.keyword, args.sorting,
                               args.page, args.max_pages)
        print_search(result)
        if args.search_download:
            for album in result:
                ...
                album.fetch_pictures()
                formmatted_foldername = format_foldername(
                    album, args.foldername_format)
                album_folder = Path.joinpath(
                    args.output_dir, formmatted_foldername)
                album.download(downloader, album_folder)
                if args.gen_pdf:
                    generate_pdf(args.output_dir,
                                 formmatted_foldername, album_folder)
